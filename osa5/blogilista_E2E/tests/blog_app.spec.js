...
const { loginWith, createBlog, likeBlog, deleteBlog, checkDeleteButtonVisible } = require('./helper');
...
describe('Blog app', () => {
  ...
  describe('When logged in', () => {
    ...
    test('the user who created the blog can delete it', async ({ page }) => {
      ...
      await expect(page.getByText(`${testblog.title} ${testblog.author}hide`)).not.toBeVisible()
    })
    ...
    test('only the user who created the blog can see the delete button', async ({ page, request }) => {
      ...
      await expect(page.getByText('Test User (testuser) logged in')).toBeVisible()
      await expect(page.getByText('Test Title Test Authorview')).not.toBeVisible();
    })
  })
